...
### [dramatiq-asyncio-memory-leak-followup](./dramatiq-asyncio-memory-leak-followup/)

PR-ready follow-up to the dramatiq-memory-leak investigation. Provides **unit tests demonstrating the bug** (memory grows 6x during retries), **unit tests proving the fix works** (memory stays stable at 1x), and a **comprehensive technical explanation** of why the leak occurs (coroutine context, traceback chains, asyncio futures, event loop state) and how the fix resolves it (exception capture, explicit cleanup, forced GC). Designed as a clean package for upstream contribution to Dramatiq with clear test output showing the problem (peak memory: 385 MB) and solution (peak memory: 65 MB).
